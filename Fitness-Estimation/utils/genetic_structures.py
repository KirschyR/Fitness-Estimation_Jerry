...
if TYPE_CHECKING:
    from utils.genetic_entities import GeneticEntity, Gene, Haplotype, HaploidGenome, Genotype
...
class Locus(GeneticStructure['Gene']):
    ...
    def entity_type(self):
        ...
        from utils.genetic_entities import Gene
        return Gene
...
class Chromosome(GeneticStructure['Haplotype']):
    ...
    def entity_type(self):
        """Lazy import to avoid circular dependency."""
        from utils.genetic_entities import Haplotype
        return Haplotype
...
class Species(GeneticStructure['HaploidGenome']):
    ...
    def entity_type(self):
        """Lazy import to avoid circular dependency."""
        from utils.genetic_entities import HaploidGenome
    ...
    def clear_entity_cache(self) -> None:
        ...
        """
        from utils.genetic_entities import GeneticEntity
        species_id = id(self)
    ...
    def _parse_haplotype_segment_str(
        ...
        # All genes must belong to loci in the same chromosome
        chroms_found = set()
        for gene in genes:
            for chrom in self.chromosomes:
                if gene.locus in chrom.loci:
                    chroms_found.add(chrom)
                    break
        
        if len(chroms_found) != 1:
            raise ValueError(
                f"Genes {[g.name for g in genes]} belong to multiple chromosomes: "
                f"{[c.name for c in chroms_found]}. "
                f"Each haplotype segment must belong to exactly one chromosome."
            )
        
        chrom = next(iter(chroms_found))
    ...
    def get_haploid_genotype_from_str(self, haploid_str: str) -> 'HaploidGenome':
        """
        ...
        Example:
            ...
            ...     "Chr2": {"X": ["X", "x"], "Y": ["Y", "y"]}
            ... })
            >>> hap = species.get_haploid_genotype_from_str("ABC;XY")
            >>> hap = species.get_haploid_genotype_from_str("a/b/c;x/y")  # equivalent
        """
        from utils.genetic_entities import Haplotype, HaploidGenome
    ...
    # Alias for backward compatibility
    def get_haplotype_from_str(self, haplotype_str: str) -> 'HaploidGenome':
        """Alias for get_haploid_genotype_from_str (backward compatibility)."""
        return self.get_haploid_genotype_from_str(haplotype_str)
    ...
    def get_genotype_from_str(self, genotype_str: str) -> 'Genotype':
        ...
        maternal = self.get_haploid_genotype_from_str(maternal_str)
        paternal = self.get_haploid_genotype_from_str(paternal_str)
        
        return Genotype(species=self, maternal=maternal, paternal=paternal)
    
    def __repr__(self):
        # Species({'Chr1': [locus1, locus2], 'Chr2': [locus3]})
        chrom_strs = []
        for chrom in self.chromosomes:
