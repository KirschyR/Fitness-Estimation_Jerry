...
class IndexCore:
    ...
    # ---------- axis sizes and flat map ----------
    # Deprecated: numeric selector / flat-index helpers removed to simplify IndexCore.
    # Use numeric backends directly or compute flattened indices with application-specific logic.

    # ---------- export utilities ----------
    def export_genotype_index_array(self) -> np.ndarray:
        return np.asarray(list(range(self.num_genotypes())), dtype=np.int64)

    def export_haplo_index_array(self) -> np.ndarray:
        return np.asarray(list(range(self.num_haplogenotypes())), dtype=np.int64)

    def export_gamete_label_array(self) -> np.ndarray:
        return np.asarray(list(range(self.num_gamete_labels())), dtype=np.int64)

    def to_numba_payload(self) -> Dict[str, np.ndarray]:
        """Return a dictionary of arrays suitable for passing into numba.

        The returned dict contains keys ``'num_genotypes'``, ``'num_haplogenotypes'``,
        and ``'num_gamete_labels'`` which are numpy arrays of dtype ``int64``.

        Returns:
            Dict[str, numpy.ndarray]: A mapping with keys ``num_genotypes``,
            ``num_haplogenotypes`` and ``num_gamete_labels``. Each value is a
            0-d numpy array of dtype ``int64`` containing the respective counts.
        """
        return {
            "num_genotypes": np.array(self.num_genotypes(), dtype=np.int64),
            "num_haplogenotypes": np.array(self.num_haplogenotypes(), dtype=np.int64),
            "num_gamete_labels": np.array(self.num_gamete_labels(), dtype=np.int64),
        }
    ...
    def decompress_hg_glab(self, compressed_idx: int, n_glabs: int) -> Tuple[int, int]:
