...
from utils.genetic_structures import GeneticStructure, Locus, Chromosome, Species

if TYPE_CHECKING:
    from utils.base_population import Sex, IndividualType
...
class Gene(GeneticEntity[Locus]):
    ...
    def is_active(
        self,
        individual_type: Optional[Any]
    ) -> bool:
        """
        Check if this gene's drive is active for this individual type.
        
        Default implementation: active if get_modifier returns non-None.
        Subclasses can override for efficiency.
        
        Args:
            individual_type: IndividualType instance (or None)
        
        Returns:
            True if drive will modify gametes from this individual
        """
        return self.get_modifier(individual_type) is not None
...
class Genotype:
    ...
    def _compute_recombinant_haplotypes_for_chromosome(
        ...
        # Select implementation based on NumbaConfig
        use_numba = True if n_loci >= 5 else False
        
        # Compute patterns with selected implementation
        patterns, frequencies = self._get_recombination_patterns(
            n_loci=n_loci, recomb_rates=recomb_rates, use_numba=use_numba
        )
    ...
    def _get_recombination_patterns(
        ...
        """
        Compute recombination patterns, selecting implementation based on use_numba.
        ...
        """
        from utils.algorithms import compute_recombinant_haplotypes
        return compute_recombinant_haplotypes(n_loci, recomb_rates, start_maternal=True, use_numba=use_numba)
...
# Haplotype (entity bound to Chromosome structure) - no alias needed, it's the new name
# The old name was "Chromosome" (entity), but that's now a structure
