...
rho <- function(observed,expected,n) {
  ...
  # Clip probabilities to avoid log(0) = -Inf (matching Python implementation)
  eps <- 1e-10
  p.x <- max(eps, min(1 - eps, expected[1]))
  p.y <- max(eps, min(1 - eps, expected[2]))
  p.z <- max(eps, 1 - p.x - p.y)
  
  #log.a <- log(n+1) + log(n+2) - log(2)
  denom <- 1 + (p.x^n + p.y^n + p.z^n)/6 - ((1-p.x)^n + (1-p.y)^n + (p.x+p.y)^n)/2
  if (denom <= 0) {
    log.a <- log(1e-10)
  } else {
    log.a <- 2*log(n) - log(denom)
  }
  
  r <-  log.a +
    lgamma(n+1)-lgamma(x.1+1)-lgamma(x.2+1)-lgamma(x.3+1) +
    x.1*log(p.x) + x.2*log(p.y) + x.3*log(p.z)
  
  # Return finite value if calculation fails
  if (!is.finite(r)) {
    return(-1e10)
  }
...
ggsave(file.path(output_dir, "track_1.pdf"), plot = p1, width = 5, height = 3)
ggsave(file.path(output_dir, "track_2.pdf"), plot = p2, width = 5, height = 3)
ggsave(file.path(output_dir, "track_3.pdf"), plot = p3, width = 5, height = 3)
...
male_viability_fitness_values <- seq(0.5, 1, by = 0.1)    # 与 Python 一致: [0.5, 0.6, ..., 1.0]
female_viability_fitness_values <- seq(0.5, 1, by = 0.1)  # 与 Python 一致: [0.5, 0.6, ..., 1.0]
...
print(results)
write.csv(results, file.path(output_dir, "results_viability.csv"))
...
write.csv(results_fecundity, file.path(output_dir, "results_fecundity.csv"))
...
## -----------------------------------------------------------------------------
results_viability <- read.csv(file.path(output_dir, "results_viability.csv"))
results_fecundity <- read.csv(file.path(output_dir, "results_fecundity.csv"))
...
# Save plots to PDF
ggsave(file.path(output_dir, "viability_heatmap.pdf"), plot = p1, width = 3, height = 3)
ggsave(file.path(output_dir, "fecundity_heatmap.pdf"), plot = p2, width = 3, height = 3)
...
# Save plots to PDF
ggsave(file.path(output_dir, "viability_heatmap_fit.pdf"), plot = p1, width = 3, height = 3)
ggsave(file.path(output_dir, "fecundity_heatmap_fit.pdf"), plot = p2, width = 3, height = 3)
...
# Save plots to PDF
ggsave(file.path(output_dir, "Ne_v.pdf"), plot = p5, width = 3, height = 3)
ggsave(file.path(output_dir, "Ne_f.pdf"), plot = p6, width = 3, height = 3)
...
# Save plots to PDF
ggsave(file.path(output_dir, "acc_v.pdf"), plot = p7, width = 4.5, height = 4)
ggsave(file.path(output_dir, "acc_f.pdf"), plot = p8, width = 4.5, height = 4)
