...
def run_tick(
    ...
    Args:
        state_arrays: dict with 'individual_count', 'sperm_storage', 'female_occupancy', 'tick'
            或 tuple 形式 (individual_count, sperm_storage, female_occupancy, tick)
        config: dict with all params and static data
            或 tuple 形式（由 config_dict_to_tuple 生成）
    ...
    """
    # Accept either dict or tuple for state/config; normalize to tuple for internal use
    if isinstance(state_arrays, dict):
        state_t = state_dict_to_tuple(state_arrays)
    else:
        state_t = state_arrays

    if isinstance(config, dict):
        config_t = config_dict_to_tuple(config)
    else:
        config_t = config

    # 工作副本（按 tuple 索引访问）
    ind_count = state_t[STATE_INDIVIDUAL_COUNT].copy()
    sperm_store = state_t[STATE_SPERM_STORAGE].copy()
    female_occ = state_t[STATE_FEMALE_OCC].copy()

    n_ages = config_t[CFG_N_AGES]
    n_gen = config_t[CFG_N_GENOTYPES]
    adult_ages = config_t[CFG_ADULT_AGES]
    ...
    s_new = alg.compute_new_sperm_pool(
        mating_prob,
        config_t[CFG_MALE_FECUNDITY],
        config_t[CFG_MEIOSIS_M], # genotype_to_gametes_map for male
        n_gen,
        config_t[CFG_N_HAPLOTYPES],
        config_t[CFG_N_GLABS],
    )
    ...
    sperm_store, female_occ = alg.update_sperm_and_occupancy(
        ...
        female_occ,
        s_new,
        config_t[CFG_ADULT_FEMALE_MATING_RATE],
        config_t[CFG_SPERM_DISPLACEMENT_RATE],
    ...
    n_0_female, n_0_male = alg.generate_offspring_distribution(
        female_counts,
        sperm_store,
        config_t[CFG_FEMALE_FECUNDITY],
        config_t[CFG_MEIOSIS_F],
        config_t[CFG_HAPLO_TO_GENOTYPE],
        config_t[CFG_OFFSPRING_PER_FEMALE],
        adult_ages[0] if adult_ages else 0,
        n_ages,
        n_gen,
        config_t[CFG_N_HAPLOTYPES],
        config_t[CFG_N_GLABS],
        config_t[CFG_SEX_RATIO],
    )
    ...
    ind_count[:, :, :] = alg.apply_age_based_survival(
        ...
        config_t[CFG_FEMALE_SURVIVAL],
        config_t[CFG_MALE_SURVIVAL],
        n_gen,
        n_ages,
    )
    ...
    ind_count[:, :, :] = alg.apply_viability(
        (ind_count[0], ind_count[1]),
        config_t[CFG_FEMALE_VIABILITY],
        config_t[CFG_MALE_VIABILITY],
        n_gen,
        n_ages,
        target_age=1,
    )
    ...
    # 2. 应用遗传漂变
    if config_t[CFG_EFFECTIVE_POP_SIZE] > 0:
        age_0 = (ind_count[0, 0, :], ind_count[1, 0, :])
        drifted = alg.apply_genetic_drift(
            age_0, config_t[CFG_EFFECTIVE_POP_SIZE], n_gen, seed=seed, counter=counter)
        ind_count[0, 0, :] = drifted[0]
        ind_count[1, 0, :] = drifted[1]
    
    # 3. 应用环境容量限制
    ind_count[:, 0, :] = alg.recruit_juveniles(
        (ind_count[0, 0, :], ind_count[1, 0, :]),
        config_t[CFG_CARRYING_CAPACITY],
        n_gen
    )
